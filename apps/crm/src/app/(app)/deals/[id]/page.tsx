...
function evidenceFields(payload: Record<string, unknown>): [string, unknown][] {
  ...
  if(payload['event']==='cash_quote_saved')return [['Action','Cash quotation saved']];
  if(['discount_requested','discount_decided'].includes(String(payload['event'])))return [['Action',payload['event']==='discount_requested'?'Discount requested':`Discount ${payload['decision']}`],['Reason',payload['reason']]];
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
      <div className="grid gap-4">
        ...
        {holds(session,'deal.read') && <Card title="Quotations"><Link href={`/deals/${id}/quotes`} className="inline-flex min-h-11 items-center text-link">Prepare or view cash quotations →</Link></Card>}
        <Card title="Discount approval"><Link href={`/deals/${id}/discount`} className="inline-flex min-h-11 items-center text-link">Request or review a discount →</Link></Card>
