...
| | |
|---|---|
| **Current module** | **`apps/crm` — the dealer application.** The shell and the M13 appraisal screens are built and run. Next: authentication (there is none), then the appraisal capture flow, then M14. |
| **Last shipped** | M13 + the CRM app (3 Aug 2026). **1,023 tests green, 208 isolation, lint 0, typecheck 0.** Also made `pnpm db:migrate`, `pnpm lint` and `pnpm typecheck` work — none of the three had ever run. |
...
There is **no login**. `CRM_DEV_USER` picks which seeded user you act as, and
the app refuses to start with `NODE_ENV=production`. Switch to
`sales@kenningtoncarsales.co.uk` to see the cost-price redaction working — that
role has no `vehicle.cost.read`, so the offer breakdown and the trade valuation
are never sent to the browser rather than hidden in it.
...
| **The CRM has no authentication.** `apps/crm/src/session.ts` resolves a development identity from `CRM_DEV_USER` and refuses to run when `NODE_ENV=production`. Permissions, site scope and RLS are all real — only the *identity* is stubbed. | **Open, and the single blocker on the CRM going anywhere near a dealer.** M2's identity tables exist (users, tenant_memberships, roles, user_sites); no login flow, password hashing, session cookie or provider does. This is the next slice. |
