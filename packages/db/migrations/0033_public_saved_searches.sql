-- Browser-private searches; no alert enrolment. Additive, records retained on rollback.
BEGIN;
ALTER TABLE saved_searches ADD COLUMN removed_at timestamptz;
CREATE INDEX saved_searches_browser_active_idx ON saved_searches (tenant_id,shortlist_id,canonical_path) WHERE removed_at IS NULL;
...
BEGIN
  ...
  -- Browser-owned searches must never grant access to contact identities or consent.
  IF to_regclass('public.saved_searches') IS NOT NULL THEN
    EXECUTE 'DROP POLICY IF EXISTS saved_search_visitor ON saved_searches';
    EXECUTE $p$CREATE POLICY saved_search_visitor ON saved_searches AS RESTRICTIVE TO app_public
      USING (contact_id IS NULL AND consent_id IS NULL AND consent_version IS NULL AND last_notified_at IS NULL
        AND EXISTS (SELECT 1 FROM shortlists s WHERE s.id = shortlist_id AND s.tenant_id = saved_searches.tenant_id))
      WITH CHECK (contact_id IS NULL AND consent_id IS NULL AND consent_version IS NULL AND last_notified_at IS NULL
        AND EXISTS (SELECT 1 FROM shortlists s WHERE s.id = shortlist_id AND s.tenant_id = saved_searches.tenant_id))$p$;
    EXECUTE 'GRANT SELECT, INSERT, UPDATE ON saved_searches TO app_public';
  END IF;
